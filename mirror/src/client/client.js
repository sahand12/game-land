 ...
 * createDispatchers
 *
 * Create action dispatchers wrappers with bound playerID and credentials
 */
function createDispatchers(
  ...
  return innerActionNames.reducer((dispatchers, name) => {
...
class _ClientImpl {
  constructor(
    ...
    this.reset = () => this.store.dispatch(ActionCreators.reset(initialState));
    this.undo = () => this.store.dispatch(ActionCreators.undo());
    this.redo = () => this.store.dispatch(ActionCreators.redo());
     ...
     *
     * Reducers generate deltalog, which are long events that are the result of
     * application of a single action.
     * The master may also send back a deltalog or the entire log depending on
     * the type of result.
     * The middleware below takes care of all these cases while managing the log
     * object.
     */
    const LogMiddleware = store => next => action => {
      ...
      switch (action.type) {
        ...
        case Actions.UPDATE: {
          ...
          // Filter out actions that are already present in the current log
          // This may occur when the client adds an entry to the log followed
          // by the update from master here.
  ...
  getState() {
    ...
    const {isConnected} = this.transport;
    ret = {...ret, isConnected};
...
export { createEventDispatchers, createMoveDispatchers };
